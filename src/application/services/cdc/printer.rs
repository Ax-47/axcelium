...
#[async_trait]
pub trait PrinterConsumerService: Consumer + Send + Sync {}
impl PrinterConsumerService for PrinterConsumerServiceImpl {}
#[async_trait]
trait PrinterService: Send + Sync {
    async fn execute(&mut self, data: CDCRow<'_>) -> anyhow::Result<()>;
}

#[async_trait]
impl PrinterService for PrinterConsumerServiceImpl {
    async fn execute(&mut self, data: CDCRow<'_>) -> anyhow::Result<()> {
