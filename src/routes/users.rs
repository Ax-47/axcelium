use crate::application::controllers::user_handle::{
    ban_user_handle, create_user_handle, delate_user_handle, get_user_count_handle,
    get_user_handle, get_users_handle, unban_user_handle, update_user_handle,
};
...
pub fn configure(cfg: &mut ServiceConfig, container: Arc<Container>) {
    ...
    cfg.service(
        web::scope("/users")
            ...
            .route("/{user_id}/mfa", web::delete().to(delate_user_handle)),
    );
}
